...
export default function ColumnPinningDynamicRowHeight() {
  ...
  const handlePreviewClick = async (demand) => {
    ...
    try {
        const response = await axios.get(`http://human-resource-management-backend.vercel.app:4000/agent/agents/${demand.professeur}`);
  ...
  const handleApprouverClick = async (demand) => {
    ...
    if (selectedDemand) {
      try {
        const response = await axios.put(`http://human-resource-management-backend.vercel.app:4000/demandes/updateStatut/${selectedDemand._id}`, {
  ...
  const handleValiderClick = async (demand) => {
    setSelectedDemand(demand);
      try {
        ...
        const response = await axios.put(`http://human-resource-management-backend.vercel.app:4000/demandes/updateStatut/${demand._id}`, {
          statut: 'Validée', // Set the new statut here
        });

        const res = await axios.post('http://human-resource-management-backend.vercel.app:4000/notifs/add-notification', { "prof": demand.professeur , "title": demand.__t.replace(/([A-Z])/g, ' $1').trim()+" Accepté", "message": "Nous tenons à vous informer que votre "+demand.__t.replace(/([A-Z])/g, ' $1').trim()+" a été accepté et que vous pouvez le récupérer à l'administration. Merci !", "date": currentDatetime});
  ...
  const handleRejeterClick = async (demand) => {
    setSelectedDemand(demand);
      try {
        ...
        const res = await axios.post('http://human-resource-management-backend.vercel.app:4000/notifs/add-notification', { "prof": demand.professeur , "title": demand.__t.replace(/([A-Z])/g, ' $1').trim()+" Refusé", "message": "Nous tenons à vous informer que votre "+demand.__t.replace(/([A-Z])/g, ' $1').trim()+" a été refusé. Merci !", "date": currentDatetime});
  ...
  const fetchDemandes = async () => {
    try {
      const response = await axios.get(`http://human-resource-management-backend.vercel.app:4000/demandes/enAttenteDemands`);
